...
class QuestionsControllerTest < ActionController::TestCase
	...
	setup do
		...
		@question_user2 = questions(:three)
	end
	...
	test "new question has to belong to current user" do
		sign_in users(:user2)
		get :new
		assert assigns(:question).user_id == users(:user2).id, "Η ερώτηση δεν ανήκει στον φοιτητή"
	end
	test "should get edit to own question signed in" do
		sign_in users(:user2)
		get :edit, :id => @question_user2.to_param
		assert_response :success
	end
	...
	test "should create question signed in" do
		...
		assert_difference('Question.count', 1, "Question count has not changed") do
			post :create, :question => { :user_id => users(:user2).id, :title => 'Title', :body => 'Body' }
		end
	...
	test "should update question signed in" do
		sign_in users(:user2)
		put :update, :id => @question_user2.to_param, :question => @question_user2.attributes
		assert_redirected_to question_path(assigns(:question))
	end
	...
	test "should destroy question signed in" do
		sign_in users(:user2)
			assert_difference('Question.count', -1) do
				delete :destroy, :id => @question_user2.to_param
			end
			assert_redirected_to questions_path
		end
